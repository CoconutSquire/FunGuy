...
The project is an early playable prototype. Local onboarding, summoning, campaign claims, save recovery, canonical stat/evolution/biome calculations, the twelve-space board, interactive combat sessions, representative effects/passives/synergies, landscape event playback and gold-to-level upgrades are implemented. Step 4 now adds ten authored draft kits and a seven-stage kitchen chapter/boss, described in SLICE_CONTENT.md. Named canonical-profile examples remain separate source references outside summon pools. Current validation and exact Android APK coverage are recorded in PROJECT_LOG.md; final art and human slice pacing remain unfinished.
...
The user requested proceeding through the build order one item at a time. Items **1: gameplay contract**, **2: combat foundation** and **3: landscape battle presentation** are complete. Step 3 passed 109/109 EditMode and 5/5 PlayMode tests, rendered layout review, a non-development Android build and emulator touch/persistence checks. The emulator had system-service startup stalls, so physical-device performance/stability remains a release gate; see PROJECT_LOG.md for precise limits. **4: authored roster, campaign/boss and one upgrade loop remains active for human pacing and content polish**. The upgrade operation, ten draft kits, seven-stage chapter and first-upgrade guidance are implemented; the complete chapter now passes native Android functional checks, including boss, practice rewards and restart persistence. Human discoverability/pacing and bespoke art/audio remain unfinished; items 5-7 remain pending. This sequence is separate from the older milestone numbering below.
...
**Current stat implementation (updated 2026-09-15):** `COMBAT_RULES.md` and versioned `stat_rules.json` define canonical unequipped stat/evolution and biome-damage rules. They supersede the initial proposals below for level indexing, SPD and biome-less behavior. Six class tables and 14 named Tank stat rows are implemented as references. Named examples use explicit stat-profile IDs; the ten existing collectible identities now use explicitly authored draft bases and kits with canonical class growth through class-growth-v1. R fighters are biome-less with 500 BST; elemental SR/UR drafts have 600 BST, or 575 for Kitchen. SLICE_CONTENT.md distinguishes source rules from authored decisions. Equipment/Core remain later progression work; formation and representative synergy semantics are defined in FORMATION_RULES.md and COMBAT_EFFECTS.md. Consult PROJECT_LOG.md for executed tests rather than treating implementation as release verification.
