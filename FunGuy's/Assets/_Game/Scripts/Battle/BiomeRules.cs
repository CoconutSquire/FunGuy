...
public enum Biome { Forest, Wetlands, Decay, Tundra, Kitchen, BiomeLess }
...
public static class BiomeRules
{
    public static Biome Parse(string value) => (value ?? "").Trim().ToLowerInvariant() switch
    {
        ...
        "kitchen" => Biome.Kitchen, "biome-less" => Biome.BiomeLess,
    ...
    public static decimal DamageMultiplier(Biome attacker, Biome defender)
    {
        ...
        if (attacker == Biome.Kitchen || defender == Biome.Kitchen || attacker == Biome.BiomeLess || defender == Biome.BiomeLess) return 1m;
