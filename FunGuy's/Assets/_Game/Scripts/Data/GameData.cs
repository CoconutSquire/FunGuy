...
public class GameData {
  ...
  public void LoadAll() {
    ...
    LegacyCharacterImporter.AppendMissingCharacters(cfile);
  ...
  private void NormalizeBanners(List<BannerDef> banners) {
    ...
    foreach (var b in banners) {
      ...
      if (total <= 0f) { b.rates = new List<RateEntry> { new() { rarity = 3, rate = .8f }, new() { rarity = 4, rate = .17f }, new() { rarity = 5, rate = .03f } }; total = 1f; }
      if (Math.Abs(total - 1f) > .0001f) foreach (var entry in b.rates) entry.rate /= total;
    }
  }

  private static string MapElementToBiome(string element) => (element ?? string.Empty).Trim().ToLowerInvariant() switch {
    "nature" => "Forest", "rot" => "Decay", "frost" => "Tundra", "ember" => "Wetlands", "astral" => "Kitchen", _ => "Kitchen",
  };
}
