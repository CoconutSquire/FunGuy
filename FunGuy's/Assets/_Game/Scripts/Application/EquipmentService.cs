...
public sealed class EquipmentService
{
    ...
    public static void ApplyTo(CombatUnit fighter, OwnedUnit owned) {
        ...
        foreach (var item in owned.gearSlots) {
            ...
            float scale = (1f + (settings?.equipmentLevelStatGrowth ?? .1f) * (Math.Max(1, item.level) - 1)) * (1f + (settings?.equipmentRarityStatGrowth ?? .2f) * (Math.Max(1, item.rarity) - 1));
            switch (item.slotId) {
    ...
    private static void ApplySymbiote(CombatUnit fighter, GearSlotState item) {
        int amount = (int)Math.Round(item.rolledStatAmount * (1f + (settings?.equipmentLevelStatGrowth ?? .1f) * (Math.Max(1, item.level) - 1)));
