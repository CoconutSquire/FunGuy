...
public static class UpgradePanelView
{
    public static UpgradePanelController Create(Transform parent, Action onClosed)
    {
        ...
        Dropdown Dropdown(string name, Transform owner, string caption, float x, float y, float w, float h, Color color)\n        {\n            var image = Panel(name, owner, x, y, w, h, color);\n            var dropdown = image.gameObject.AddComponent<Dropdown>();\n            dropdown.targetGraphic = image;\n            dropdown.captionText = Label(name + "Caption", image.transform, caption, 12, 0, w - 42, h - 8, 18, Color.white);\n            dropdown.captionText.alignment = TextAnchor.MiddleLeft;\n            var arrow = Label(name + "Arrow", image.transform, "▼", w / 2f - 28, 0, 24, h - 8, 14, Color.white);\n            arrow.alignment = TextAnchor.MiddleCenter;\n            var templateRect = Rect(name + "Template", image.transform, 0, -h / 2f, w, 180);\n            templateRect.pivot = new Vector2(0.5f, 1f);\n            templateRect.gameObject.SetActive(false);\n            var templateImage = templateRect.gameObject.AddComponent<Image>();\n            templateImage.color = new Color(.07f, .11f, .10f, 1f);\n            var item = new GameObject("Item", typeof(RectTransform), typeof(Toggle));\n            item.transform.SetParent(templateRect, false);\n            var itemRect = item.GetComponent<RectTransform>();\n            itemRect.anchorMin = new Vector2(0, 1); itemRect.anchorMax = new Vector2(1, 1);\n            itemRect.pivot = new Vector2(.5f, 1); itemRect.sizeDelta = new Vector2(0, 42);\n            var itemImage = item.AddComponent<Image>(); itemImage.color = new Color(.12f, .22f, .20f, 1f);\n            var itemText = new GameObject("ItemLabel", typeof(RectTransform)).AddComponent<Text>();\n            itemText.transform.SetParent(item.transform, false);\n            var itemTextRect = itemText.rectTransform; itemTextRect.anchorMin = Vector2.zero; itemTextRect.anchorMax = Vector2.one; itemTextRect.offsetMin = new Vector2(12, 0); itemTextRect.offsetMax = new Vector2(-12, 0);\n            itemText.font = font; itemText.fontSize = 18; itemText.color = Color.white; itemText.alignment = TextAnchor.MiddleLeft;\n            var toggle = item.GetComponent<Toggle>(); toggle.targetGraphic = itemImage; toggle.graphic = null;\n            dropdown.template = templateRect; dropdown.itemText = itemText;\n            return dropdown;\n        }\n\n        var rosterColor = new Color(.10f, .25f, .34f, 1f);
